...
export async function runScan(scanId, repoUrl, deployUrl) {
  ...
  try {
    ...
    try {
      ...
      if (repoUrl && repoUrl.includes('github.com')) {
        ...
        if (rmRes.ok) {
          ...
          emit("No README.md found.", 15, true);
        }
        
        emit("Reading package.json", 20);
        ...
        if (pkgRes.ok) {
          ...
          emit("No package.json found.", 20, true);
        }
        ...
        emit(`Generating architecture`, 30);
      } else {
        emit("No valid GitHub URL provided, skipping repository analysis.", 15, true);
      }
    } catch (e) {
    ...
    if (deployUrl) {
      ...
      try {
        ...
        const captureNode = async (currentUrl, pathName, startTime) => {
          ...
          try {
            ...
            await db.insert('screenshots', { scan_id: scanId, path: pathName, url: b64 });
          } catch(e) { 
          ...
          try {
            ...
            throw new Error(`Accessibility analysis failed: ${e.message}`);
          }
        ...
        emit("Collecting console logs", 70);
        emit("Collecting network logs", 75);
      ...
      emit("Skipping Playwright (No Deployment URL)", 75, true);
    }
    
    // 3. AI ANALYSIS
    try {
      emit("Detecting issues", 85);
      ...
      // 4. PERSIST RESULTS
      emit("Saving results to Supabase", 95);
    ...
    await db.update('scans', { status: 'failed', error_message: errorMsg }, { id: scanId }).catch(()=>{});
